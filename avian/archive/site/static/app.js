...
function renderToday(data) {
  ...
  holder.innerHTML = data.species.map((bird, index) => { const slug = bird.slug || bird.art_slug; const artSlug = bird.art_slug || bird.slug; return `<a class="bird-card" href="${escapeHTML(speciesHistoryHref(slug))}"><span class="bird-number">${String(index + 1).padStart(2, "0")}</span><img src="${escapeHTML(artURL(artSlug))}" alt="Illustration of ${escapeHTML(bird.common_name)}"><h2>${escapeHTML(bird.common_name)}</h2><em>${escapeHTML(bird.scientific_name)}</em><span class="bird-facts">${formatNumber(bird.detections)} recognition${Number(bird.detections) === 1 ? "" : "s"} · ${displayTime(bird.first_heard)}–${displayTime(bird.last_heard)}</span></a>`; }).join("");

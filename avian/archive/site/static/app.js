...
function renderBirdCard(bird, detailed = false) {
  ...
  const conflictCount = Number(counts.model_conflict) || 0;
  const conflictLabel = conflictCount ? ` · ${formatNumber(conflictCount)} model-conflict candidate${conflictCount === 1 ? "" : "s"}` : "";
  const recognitionLabel = `${formatNumber(bird.detections)} accepted recognition${Number(bird.detections) === 1 ? "" : "s"}${conflictLabel}`;
  const header = `<header class="field-card-header"><div class="field-card-masthead"><a class="field-card-name" href="${escapeHTML(href)}"><h3>${escapeHTML(bird.common_name)}</h3><em>${escapeHTML(bird.scientific_name)}</em></a><div class="field-card-badges"><span class="garden-rarity">${escapeHTML(rarity)}</span><span class="species-standing ${standing}">${escapeHTML(standingLabel)}</span></div></div><div class="field-card-portrait"><img src="${escapeHTML(artURL(bird.art_slug || bird.slug))}" alt="Illustration of ${escapeHTML(bird.common_name)}"><span class="field-card-art-fallback" role="img" aria-label="No independently verified illustration available for ${escapeHTML(bird.common_name)}"><b>Voice-first species</b><small>No verified illustration</small></span></div><p class="field-card-observation"><span>Garden record</span><strong>${escapeHTML(recognitionLabel)}</strong></p></header>`;
  if (card.research_status !== "verified" || !card.facts) {
    return `<article class="field-card research-pending ${standing} ${detailed ? "field-card-detailed" : ""}">${header}<div class="field-card-pending"><span class="field-card-pending-kicker">Catalogue desk</span><strong>Research pending</strong><p>This newly heard species is visible immediately; its natural-history fields will appear only after source review passes.</p></div></article>`;
  }
  ...
  const notesOpen = detailed ? " open" : "";
  const notesAction = detailed ? "Field notes" : "Open field notes";
  return `<article class="field-card ${standing} ${detailed ? "field-card-detailed" : ""}">${header}<div class="field-card-facts"><dl class="card-essentials"><div><dt>Habitat</dt><dd>${escapeHTML(facts.habitat.value)}</dd></div><div><dt>Diet</dt><dd>${escapeHTML(facts.diet.value)}</dd></div></dl><div class="card-stat-rail"><div><strong>${escapeHTML(formatFactRange(facts.wingspan_cm, " cm"))}</strong><small>wingspan</small></div><div><strong>${escapeHTML(formatFactRange(facts.length_cm, " cm"))}</strong><small>length</small></div><div><strong>${escapeHTML(formatFactRange(facts.clutch_size))}</strong><small>clutch</small></div></div><blockquote>${escapeHTML(facts.fact.value)}</blockquote><details class="card-field-notes"${notesOpen}><summary><span>${escapeHTML(notesAction)}</span><small>Nest · movement · conservation · sources</small></summary><div class="card-field-notes-body"><dl class="card-compact-facts"><div><dt>Nest · ${escapeHTML(facts.nest.type)}</dt><dd>${escapeHTML(facts.nest.value)}</dd></div><div><dt>Movement · ${escapeHTML(facts.migration.category)}</dt><dd>${escapeHTML(facts.migration.value)}</dd></div><div><dt>Conservation</dt><dd>${escapeHTML(facts.conservation.system)} ${escapeHTML(facts.conservation.status)} · assessed ${escapeHTML(facts.conservation.assessed_at)}</dd></div>${taxonomy}</dl>${sourceLinks(card)}<p class="card-review-proof">Catalogue ${escapeHTML(card.catalogue_version)} · independently reviewed by ${formatNumber(reviewers)} models</p></div></details></div></article>`;
}
...
function renderSpecies(holder, species, candidate = false) { if (!species.length) { holder.innerHTML = `<p class="empty-state">${candidate ? "No uncorroborated species candidates." : "No species have been independently corroborated yet."}</p>`; return; } const max = Math.max(1, ...species.map(item => Number(item.days_heard))); holder.innerHTML = species.map((bird, index) => { const counts = bird.review_counts || {}; const conflicts = Number(counts.model_conflict) || 0; const warning = candidate ? `<span class="species-standing ${conflicts ? "model_conflict" : "uncorroborated"}">${conflicts ? "Model conflict" : "Uncorroborated"}</span>` : '<span class="species-standing corroborated">Corroborated</span>'; const dayLabel = bird.days_heard === 1 ? "day" : "days"; const recognitionLabel = bird.detections === 1 ? "recognition" : "recognitions"; const conflictLabel = conflicts ? `<small>${formatNumber(conflicts)} model-conflict candidate${conflicts === 1 ? "" : "s"} excluded</small>` : ""; return `<a class="species-row" href="${escapeHTML(speciesHistoryHref(bird.slug))}"><span class="species-rank">${String(index + 1).padStart(2, "0")}</span><span class="species-name"><strong>${escapeHTML(bird.common_name)}</strong><em>${escapeHTML(bird.scientific_name)}</em>${warning}</span><span>${formatNumber(bird.days_heard)} ${dayLabel} heard</span><i aria-hidden="true"><b style="width:${Math.max(3, Number(bird.days_heard) / max * 100)}%"></b></i><span>${formatNumber(bird.detections)} accepted ${recognitionLabel}${conflictLabel}</span></a>`; }).join(""); }
...
async function initSpeciesDetail(route) {
  ...
  try {
    ...
    const conflictCount = Number(bird.review_counts?.model_conflict) || 0;
    instancesLink.textContent = `View evidence: ${formatNumber(bird.detections)} accepted recognition${Number(bird.detections) === 1 ? "" : "s"}${conflictCount ? ` · ${formatNumber(conflictCount)} model-conflict candidate${conflictCount === 1 ? "" : "s"}` : ""} ↓`;
...
function reviewText(item) { const status = String(item.review_status || "unreviewed"); if (!item.review_model) return status === "pending" ? "Pending independent review" : "Not independently reviewed"; const rank = item.review_rank ? ` · rank #${formatNumber(item.review_rank)} of ${formatNumber(item.review_label_count)}` : ""; const score = item.review_score == null ? "" : ` · claimed-species model score ${formatConfidence(item.review_score)}`; const leader = item.review_top_label && item.review_top_score != null && item.review_top_label !== item.scientific_name ? ` · leading alternative ${item.review_top_label} ${formatConfidence(item.review_top_score)}` : ""; const provenance = item.review_top_score_provenance === "stored_review_notes_0.1pct" ? " · alternative score reconstructed from stored 0.1%-rounded notes" : ""; return `${item.review_model} · ${status.replace("_", " ")}${rank}${score}${leader}${provenance} · model score—not probability`; }
